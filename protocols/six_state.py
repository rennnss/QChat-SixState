"""Six‑State QKD Protocol implementation.

Uses three mutually unbiased bases (Z, X, Y) for qubit preparation and
measurement.  The expected sifting rate is ≈ 1/3 and an intercept‑resend
attack introduces a QBER of ≈ 1/3.
"""
...
from quantum.utils import SIX_STATE_BASES


class SixStateProtocol(QKDProtocol):
    ...
    PROTOCOL_NAME = "Six-State"
    QBER_THRESHOLD = 1.0 / 3.0  # Theoretical max QBER from Eve
    ...
    def run(
        ...
        """Execute one round of the Six‑State protocol.

        Pipeline:
        1. Alice prepares qubits in random bases (Z, X, Y).
        2. (Optional) Eve intercepts and re-sends.
        3. Bob measures qubits in random bases (Z, X, Y).
        4. Basis reconciliation – keep matching bases (≈ 1/3).
        ...
        """
        # ── 1. Alice prepares ─────────────────────────────────────────────
        alice = Alice(basis_set=SIX_STATE_BASES, seed=self._alice_seed)
        alice_state = alice.prepare_qubits(n_qubits)

        # ── 2. Eve intercepts (optional) ──────────────────────────────────
        circuits = alice_state.circuits
        if eve_present:
            eve = Eve(basis_set=SIX_STATE_BASES, seed=self._eve_seed)
        ...
        bob = Bob(basis_set=SIX_STATE_BASES, seed=self._bob_seed)
        ...
        # ── 6. Detection ─────────────────────────────────────────────────
        eve_detected = qber > (self.QBER_THRESHOLD * 0.5)  # conservative
